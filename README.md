...
Независимый IPTV-плагин для Lampa 3.x, версия 0.3.0. Он не использует CUB API и хранит адреса плейлиста/телегида локально в настройках Lampa.

## Возможности

- загрузка M3U/M3U8 по URL;
- плиточный интерфейс в стиле OTTPlayer: номера и логотипы каналов, текущая передача, время и шкала прогресса;
- нижняя строка категорий и переключение каналов в IPTV-интерфейсе Lampa;
...
Кнопка **Телепрограмма** задаёт отдельный URL XMLTV, **↻ Обновить** повторно загружает плейлист и EPG. Обычное нажатие на плитку запускает эфир, долгое нажатие открывает телепрограмму и доступный архив.
